...
- For stable Home dashboard, dashboard widget, static page, status strip, selector/detail, and repeated fragment structures, consider `Template`, `SlotKey`, and per-request `RenderContext`. Use them when the DOM shape is stable and only labels, counts, hrefs, chips, statuses, or bounded child fragments change.
- New SlotKey/template key helper or key-bundle value types must expose concise `.of(...)` factories. Do not scatter direct `new` construction through controllers or render methods when a named helper keeps call sites readable.
- Do not share mutable SimplyPages component instances across requests. Reuse immutable `Template` objects and feed each request/render flow through a fresh `RenderContext` unless confinement is explicit and documented.
- Preserve HTMX-first interactions, stable swap roots, fallback `href`s, and existing route behavior when introducing template reuse.
- Before changing Home dashboard styling or layout, read `.internal-dev/specifications/web.md`, `.internal-dev/specifications/simplypages.md`, and the dashboard layout knowledge files, then preserve the `/manage` plus `/agents` operational style language unless the user explicitly changes direction.
- For the Home dashboard, the rendered dashboard is the source of truth for layout. Move, resize, add-row, add-widget, and remove controls should decorate the live dashboard surface; modal or drawer flows are only for module-specific detail/settings work.
- For Home dashboard layout work, read `.internal-dev/knowledge/simplypages-avatar-layout-and-editing.md` and compare the implementation against SimplyPages `Row`, `Column`, `EditableModule`, HTMX, OOB, and slot-key patterns before changing code. Treat `AvatarDashboard*` class names as legacy implementation names until a deliberate rename is planned.
- Home dashboard edit mode should resemble the SimplyPages HTMX editing demo: widget content remains primary, decorator controls sit in the top corner, add-widget controls are centered between row content and row controls, and insert-row controls are quiet separators. Do not reintroduce a layout-list modal or large widget-internal movement/resize panels as the primary editor.
...
- For Home dashboard UI changes, Playwright validation must capture normal and edit mode on desktop and mobile, compare against `/manage`, `/agents`, and the SimplyPages editing demo when layout editing is touched, and explicitly report alignment, dead space, density, hierarchy, wrapping, and control affordance quality.
