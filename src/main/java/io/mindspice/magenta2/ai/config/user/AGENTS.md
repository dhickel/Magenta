...
- Represent configured models, agents, summarization agent selection, context buffer policy, endpoint types, prompts, approved tool names, web search settings, and per-agent shell command allowlists.
...
- Keep this guide updated when the config file shape, validation rules, supported endpoint types, web search settings, or wildcard semantics change.
