...
const outDir = "artifacts/dashboard-widget-suite/phase-02-browser-rerun";
...
const results = {
  ...
  runtime: {
    ...
    port: 18080,
    isolatedRoot: "/tmp/magenta2-dashboard-widget-suite-phase-02-browser-rerun"
  },
  screenshots: [],
  checks: [],
  console: [],
...
try {
  ...
  const reminderTitle = `Browser UI reminder ${Date.now()}`;
  await page.locator('#avatar-widget-detail-modal form[hx-post="/_dashboards/_reminders"] input[name="title"]').fill(reminderTitle);
  await page.locator('#avatar-widget-detail-modal form[hx-post="/_dashboards/_reminders"] input[name="remindAt"]').fill(localDateTime(today, 16, 15));
  const reminderNotes = page.locator('#avatar-widget-detail-modal form[hx-post="/_dashboards/_reminders"] textarea[name="notes"]');
  if (await reminderNotes.count()) {
    await reminderNotes.fill("Browser proof reminder form");
  }
  await Promise.all([
    page.waitForResponse((response) => response.url().includes("/_dashboards/_reminders")),
    page.locator('#avatar-widget-detail-modal form[hx-post="/_dashboards/_reminders"] button[type="submit"]').click()
  ]);
  await page.waitForTimeout(400);
  health = await dashboardHealth(page);
  const calendarAfterReminderWidgetText = await page.locator('[data-avatar-widget-type="calendar-schedule"]').first().innerText();
  check("reminder_htmx_form_refresh_boundaries", calendarAfterReminderWidgetText.includes(reminderTitle)
    && health.topNavs === 1
    && health.dashboardHomes === 1
    && health.duplicateIds.length === 0,
  `reminder visible=${calendarAfterReminderWidgetText.includes(reminderTitle)}; duplicates=${health.duplicateIds.join(",") || "none"}`);

  await closeModal(page);
  await openDetail(page, "Open Calendar/Schedule detail");
  await screenshot(page, "desktop-calendar-after-reminder-form.png");
  ...
  check("calendar_reflects_occurrence_status_and_reminder", calendarAfterActions.toLowerCase().includes("skipped")
    && calendarAfterActions.includes(reminderTitle),
  `calendar skipped visible=${calendarAfterActions.toLowerCase().includes("skipped")}; reminder visible=${calendarAfterActions.includes(reminderTitle)}`);
  await closeModal(page);
  ...
  results.visualCritique = [
    ...
    "Calendar/Schedule uses a real month grid, agenda, and visible HTMX forms for time blocks and reminders.",
    "Today Planner hierarchy is compact and now exposes overdue, unscheduled, and quick-captured work in usable sections.",
    "Tasks/Routines filters are compact and HTMX-first, with recurrence, project, subtodo, and occurrence action details visible.",
