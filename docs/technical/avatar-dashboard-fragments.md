...
The visible explorer contract is a details/list layout with `Name`, `File Type`, `Size`, `Created`, `Last Modified`, `Tags`, and `Actions` columns plus a separate right inspector. Row actions stay compact: view when supported, rename, and delete. The inspector mirrors view/rename/delete and owns the expanded copy/move/tag controls.
...
- `/js/avatar-shell.js?v=5` owns desktop chat corner resizing and browser-local width/height persistence.
