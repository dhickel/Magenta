...
If a duplicate or stale answer arrives after the server has already consumed the active question, `ChatService` treats it as a recoverable plan-state refresh while the conversation remains in `PLAN` mode. It returns the current plan state with a recovery clarification rather than surfacing `No active planning question exists for this conversation` as a terminal browser error.
