...
After the final queued prompt answer is recorded, `ChatService` immediately asks the planning model to continue from the structured seed context. If that model request fails with a non-transient AI error such as invalid credentials, the answer remains saved and the route returns a controlled assistant message instead of propagating a servlet error. The prompt is not re-queued, so the next user message can continue planning after the model configuration is corrected.
