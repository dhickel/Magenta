...
After the final queued prompt answer is recorded, `ChatService` immediately asks the planning model to continue from the structured seed context. If that continuation fails because the model request fails or a planning tool call cannot complete, the answer remains saved and the route returns a controlled assistant message instead of propagating a servlet error. The prompt is not re-queued, so the next user message can continue planning after the unavailable model, credential, or tool dependency is corrected.
