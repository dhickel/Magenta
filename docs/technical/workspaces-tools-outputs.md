...
Non-job task and workflow assignments are also validated at assignment creation for a nonblank `runDisplayName`. Legacy rows with null names remain readable, and `JOB_RUN` submissions can continue to rely on job context for user-visible naming.
...
Tag editing is modal-driven from the inspector. The Tag Editor modal renders a compact row inventory with Directory/File filters, each row showing tag name, slug, target type, and description metadata. Row-open edit forms constrain target type to directory or file, and assignment still persists through `workspace_file_label_assignments`; tag definition metadata remains in `workspace_file_labels.metadata_json.targetType` plus optional `description`. Server-side assignment still rejects mismatched typed labels, and tag-definition deletion is not exposed.
