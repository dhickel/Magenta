...
- `POST /api/chat/turns/{turnId}/interrupt`: interrupt an active turn using the `turnId` and `interruptToken` from the stream `start` event. The browser `/chat` composer does not use this route for ordinary mid-turn messages; it uses the pending-message queue instead.
...
SSE events are JSON events. Chat emits `start`, `chunk`, `tool`, `system`, `interrupt`, `context`, `done`, and `error` from `ChatStreamEvent`; the controller also emits plan-execution updates around anonymous plan execution. The `start` payload includes an interrupt token that is accepted during plain model calls, tool model calls, and tool checkpoints. Client disconnect/error terminal paths clear active turn and saved-plan execution registrations, and disconnect/timeout cleanup interrupts the abandoned model worker so a later stream can start without a stale active-turn conflict. Chat streams also send SSE comment heartbeats as transport probes; clients should ignore comments and process only named JSON events. Cancelled stream owners are fenced from assistant persistence before retry writes proceed. Anonymous plan execution reports `planState.status=COMPLETED` only after validator-gated `plan_complete` succeeds. If automatic completion repair is exhausted, `done.planState.status` is `NEEDS_REVIEW` and the response text is a controlled review notice, not the model's unvalidated completion text.
