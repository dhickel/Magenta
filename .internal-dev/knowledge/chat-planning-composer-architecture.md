...
- Pending-message card rendering must be guarded by both the active conversation id and a monotonic pending-load generation. A queued drain can reload history while the row is still `CLAIMED`; a later ack may delete the row, so older pending-list responses must not be allowed to repaint a stale `Sending...` card after the final empty reload.
- A queued-drain stream should keep sending and acking the claimed row even if the user switches sessions, but it must not steal the visible `/chat` session back to the drained conversation.
