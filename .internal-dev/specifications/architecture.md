...
| ARCH-20260526-03 | Agent Skills MVP scope and storage boundary | active | MVP skill discovery is rooted at application-owned `<magenta-root>/skills/` (from `MagentaRootProperties.path()/skills`) with agent-profile assignment and activation only. Project-local `.agents/skills/`, user-home scopes, layered assignment beyond agent scope, and script-trust/registry/marketplace flows are deferred. | `ai/skills` phase-02 domain (`AgentSkillRepositoryService`, `AgentSkillParser`, `AgentSkillCatalogService`, `AgentSkillRepository`) and Agent Skills phase plan | Phase 02 foundation is implemented (root repository, parser/validation diagnostics, metadata persistence, refresh hashing). Assignment, runtime activation, API, and UI phases remain pending. | Parser/discovery/repository tests, startup smoke when downstream wiring phases land, and final spec-adherence validation in phase 06. | `DECISION-20260526-03`, `DECISION-20260526-04`, `DECISION-20260526-05`, `DECISION-20260526-06` | `agent-skills-specification-reference.md` |
