...
| ARCH-20260526-03 | Agent Skills MVP scope and storage boundary | active | MVP skill discovery is rooted at application-owned `<magenta-root>/skills/` (from `MagentaRootProperties.path()/skills`) with agent-profile assignment and activation only. Project-local `.agents/skills/`, user-home scopes, layered assignment beyond agent scope, and script-trust/registry/marketplace flows are deferred. | `ai/skills` domain (`AgentSkillRepositoryService`, `AgentSkillParser`, `AgentSkillCatalogService`, `AgentSkillRepository`, `AgentSkillAssignmentRepository`, `AgentSkillRuntimeCatalogService`, `AgentSkillActivationService`) and chat prompt/tool integration (`PromptContextAssembler`, `ToolAccessPolicy`, `activate_skill`) | Phase 02+03 backend is implemented (root repository, parser/validation diagnostics, metadata persistence, assignment table, runtime catalog filtering, body-only activation with resource listing, and activation dedupe). API and UI management phases remain pending. | Parser/discovery/repository tests; assignment/catalog/activation/prompt/tool tests; bounded startup; final spec-adherence validation in phase 06. | `DECISION-20260526-03`, `DECISION-20260526-04`, `DECISION-20260526-05`, `DECISION-20260526-06` | `agent-skills-specification-reference.md` |
