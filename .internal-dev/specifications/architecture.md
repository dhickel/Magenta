...
| ARCH-20260529-01 | Assistant dashboard widget platform | active | Dashboard rows store widget instances addressed by stable instance ids. Widget behavior is defined by a Java registry; widget type, instance policy, binding mode, settings defaults, renderer keys, refresh policy, empty-state policy, and tool metadata are registry-owned. Dashboards remain layout containers and do not own agents, projects, Work Areas, jobs, or outputs. | `avatar.dashboard` registry package, `user_dashboard_widgets`, `AvatarService`, `AvatarDashboardController` | Feature-rich widgets beyond migrated shells are implemented in later phases. | Registry/repository/service/controller tests, startup, and delegated Playwright proof. | `DECISION-20260529-01` | `dashboard-fragment-navigation.md`, `simplypages-avatar-layout-and-editing.md` |
| ARCH-20260529-02 | Avatar personal planning core | active | Avatar-owned planner state in `avatar.sqlite` now includes service-owned day maps, time blocks, in-dashboard reminders, occurrence status, and merged Today/Tasks/Calendar read models. Runtime assignments/jobs/projects remain in existing Magenta services and are linked by ids only. | `AvatarService`, `AvatarRepository`, `avatar-schema.sql`, planner widget renderers/tools | No cross-database foreign keys, no external notification delivery, and no automatic assignment creation from planner tasks. | Repository/service/controller/tool tests, startup, and delegated browser proof. | `DECISION-20260529-02` | `simplypages-avatar-layout-and-editing.md` |
