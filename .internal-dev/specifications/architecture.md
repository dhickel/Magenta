...
| ARCH-20260526-03 | Agent Skills MVP scope and storage boundary | active | MVP skill discovery is rooted at application-owned `<magenta-root>/skills/` (from `MagentaRootProperties.path()/skills`) with agent-profile assignment and activation only. Project-local `.agents/skills/`, user-home scopes, layered assignment beyond agent scope, and script-trust/registry/marketplace flows are deferred. | Agent Skills phase-01 specification lock and worker directive | Product code has not landed yet; all behavior here is intended contract and must remain explicitly marked pre-implementation in docs. | Spec/doc consistency checks, targeted implementation-phase tests after code lands, startup smoke once wiring exists. | `DECISION-20260526-03`, `DECISION-20260526-04`, `DECISION-20260526-05`, `DECISION-20260526-06` | `agent-skills-specification-reference.md` |
