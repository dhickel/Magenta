...
| ARCH-20260526-02 | Runtime `AGENTS.md` layering and confinement | active | Runtime `AGENTS.md` context is resolved only inside the bound project/workspace root; all ancestor files from bound root to active path remain active context; the closest file has precedence only for conflicts; explicit user/task instructions override all `AGENTS.md` content. | Workspace root binding, path confinement helpers, runtime prompt/context assembly | Closest-wins shorthand from the external site is intentionally interpreted as conflict precedence, not ancestor exclusion. | Resolver and prompt/context tests for no-file/root-only/nested-only/root-plus-nested cases plus confinement checks for traversal and symlink escape. | `DECISION-20260526-02` | `agents-md-specification-reference.md` |
