...
| WEB-20260526-02 | Agent Skills browser/editor and guided creation MVP | active | Provide operational `/skills` management surfaces for root-repository skills and per-agent assignment with HTMX-first fragments. UI must clearly separate implemented MVP behavior from deferred scope and must not imply UI script execution support. | Phase-04 minimal shell/fragments implemented at `/skills`, `/skills/_list`, `/skills/_detail/{skillName}`, `/skills/_files/{skillName}`, `/skills/_viewer/{skillName}`, `/skills/_assignments/{skillName}`; full guided editing/creation UX remains Phase 05. | Standard CRUD/filter/row actions use HTMX; JavaScript stays narrow (editor affordances only when clearly simpler). MVP only manages root `skills/` repository plus agent assignment. | Controller/fragment tests now cover baseline render paths; focused Playwright screenshots and visual critique remain required for Phase 05 full UI flow. | `DECISION-20260526-03`, `DECISION-20260526-06` | `agent-skills-specification-reference.md` |
