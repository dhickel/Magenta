...
| WEB-20260526-02 | Agent Skills browser/editor and guided creation MVP | active | Provide operational `/skills` management surfaces for root-repository skills and per-agent assignment with HTMX-first fragments. UI must clearly separate implemented MVP behavior from deferred scope and must not imply UI script execution support. | Planned skills list/editor/assignment pages and fragments | Standard CRUD/filter/row actions use HTMX; JavaScript stays narrow (editor affordances only when clearly simpler). MVP only manages root `skills/` repository plus agent assignment. | Focused Playwright screenshots and visual critique once UI exists; controller/browser checks for assignment and editor flows. | `DECISION-20260526-03`, `DECISION-20260526-06` | `agent-skills-specification-reference.md` |
