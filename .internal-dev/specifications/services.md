...
| SVC-20260526-03 | Agent Skills catalog, parsing, assignment, and activation services | active | Skill-domain services own root `skills/` discovery, `SKILL.md` parsing, metadata persistence, agent assignment, catalog disclosure payloads, and activation state. MVP activation is agent-scoped only and favors dedicated activation tooling plus file-read fallback. Script execution policies, project/user scope loading, layered assignment, and registry/package trust are deferred. | Implemented phase-02 `ai.skills` foundation: `AgentSkillRepositoryService`, `AgentSkillParser`, `AgentSkillCatalogService`, `AgentSkillRepository`; future phases extend this surface for assignment/activation/API/UI | Controllers stay thin. Chat services consume a skill catalog/activation surface instead of reimplementing parser/discovery logic. `allowed-tools` remains experimental and must not be treated as enforced policy in MVP. | Phase-02 parser/discovery/repository tests; phase-03+ tests add assignment, catalog filtering, activation, and prompt/tool integration coverage. | `DECISION-20260526-04`, `DECISION-20260526-05`, `DECISION-20260526-06` | `agent-skills-specification-reference.md` |
