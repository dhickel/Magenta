...
| SVC-20260526-03 | Agent Skills catalog, parsing, assignment, and activation services | active | Skill-domain services own root `skills/` discovery, `SKILL.md` parsing, metadata persistence, agent assignment, catalog disclosure payloads, and activation state. MVP activation is agent-scoped only and favors dedicated activation tooling plus file-read fallback. Script execution policies, project/user scope loading, layered assignment, and registry/package trust are deferred. | Planned `ai.skill` domain services, chat prompt/context assembly, chat tool integration, and orchestration-owned agent profile settings | Controllers stay thin. Chat services consume a skill catalog/activation surface instead of reimplementing parser/discovery logic. `allowed-tools` remains experimental and must not be treated as enforced policy in MVP. | Service-level parser/catalog/assignment tests, tool contract tests, prompt/context tests, and startup smoke once beans/config load. | `DECISION-20260526-04`, `DECISION-20260526-05`, `DECISION-20260526-06` | `agent-skills-specification-reference.md` |
