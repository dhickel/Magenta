...
| SVC-20260526-03 | Agent Skills catalog, parsing, assignment, activation, and file-management services | active | Skill-domain services own root `skills/` discovery, `SKILL.md` parsing, metadata persistence, assignment metadata, runtime catalog disclosure, activation state, and root-confined API-facing file operations. MVP activation is agent-scoped only and uses dedicated activation tooling with dedupe. Script execution policies, project/user scope loading, layered assignment, and registry/package trust are deferred. | Implemented phase-02+04 `ai.skills` services: `AgentSkillRepositoryService`, `AgentSkillParser`, `AgentSkillCatalogService`, `AgentSkillRepository`, `AgentSkillAssignmentRepository`, `AgentSkillAssignmentService`, `AgentSkillRuntimeCatalogService`, `AgentSkillPromptCatalogAssembler`, `AgentSkillActivationService`, `AgentSkillManagementService` | Controllers/chat remain consumers. Prompt assembly appends metadata-only catalog when assigned+enabled+loadable skills exist. Dedicated `activate_skill` returns parsed body-only instructions and resource listing without eager reads. File routes resolve skill slug/name through catalog metadata, enforce root confinement through repository service path guards, block traversal/symlink escapes, and map unsupported text operations distinctly. `allowed-tools` remains experimental and must not be treated as enforced policy in MVP. | Parser/discovery/repository tests, assignment persistence/filtering tests, activation/dedupe tests, prompt-context tests, tool-registry/tool-access tests, controller/API tests for file confinement/status mapping, and bounded startup. | `DECISION-20260526-04`, `DECISION-20260526-05`, `DECISION-20260526-06` | `agent-skills-specification-reference.md` |
