...
| SPEC-20260525-02 | `schema.sql` | active | persistence | Browser chat pending messages are stored separately from chat memory until claimed and sent as normal turns; startup normalizes legacy duplicate `(conversation_id, message_order)` rows before enforcing uniqueness so same-conversation FIFO keys cannot duplicate under concurrent enqueue. | `ai_chat_pending_messages`, `ChatPendingMessageRepository` | Schema warm-start duplicate normalization, repository FIFO, concurrent enqueue uniqueness, claim/ack/release, stale recovery, and clear-conversation tests. | none | `chat-context-management-and-tools.md` |
| SPEC-20260531-01 | `WorkflowRepository` inline workflow migrations | active | persistence | Workflow repository startup migrations are idempotent for already-existing columns and fail visibly for unexpected schema inspection or alteration failures. | `WorkflowRepository.ensureTables()`, `WorkflowRepositoryTest` | Fresh schema, warm schema, forced migration failure, and bounded startup. | none | `workflow-route-model.md` |
