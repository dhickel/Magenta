...
| API-20260529-03 | Assistant dashboard notes/project fragments and tools | active | Notes widgets expose instance-scoped personal capture/open routes plus confined file-note read/save routes. Project widgets expose project artifact update fragments for fixed artifact types. Static Avatar tools expose `avatar_file_note_read`, `avatar_file_note_update`, `avatar_project_context_get`, and `avatar_project_artifact_update`. | HTML fragments and compact JSON tool responses. | Personal note compatibility route `POST /_dashboards/_notes` remains; file note and project artifact routes require widget instance settings/bindings. | Controller/service/tool registry tests plus startup; browser proof delegated. | `DECISION-20260529-03` | `dashboard-fragment-navigation.md` |
| API-20260529-04 | Assistant dashboard agent operational fragments and tool descriptors | active | Agent operational widgets expose instance-scoped output preview `GET /dashboards/{dashboardId}/widgets/{widgetInstanceId}/_outputs/{artifactId}` and Work Area file preview `GET /dashboards/{dashboardId}/widgets/{widgetInstanceId}/_work-area-file?path=...`. The Phase 04 widget registry declares only current-context `agent_*` tool names for normal-agent operational descriptors. | HTML fragments; scoped output misses return `404`; invalid Work Area file bindings return `400` or `404`. | Legacy `GET /_dashboards/_outputs/{artifactId}` remains for compatibility and generic preview. New Work Area mini-view routes do not use legacy `/avatar/_work-areas` routes. | Controller/tool registry tests plus startup; browser proof delegated. | `DECISION-20260529-04` | `dashboard-fragment-navigation.md`, `workarea-operational-ui-consistency.md` |
| API-20260529-05 | Assistant dashboard habits, reminders, and context fragments | active | Habits expose HTMX create, log/history-correction, and archive routes. Reminders expose HTMX create, complete, snooze, reschedule, skip, and restart routes. Dashboard Context renders read-only selected dashboard/widget state and visible tool descriptor names. | `POST /_dashboards/_habits`, `POST /_dashboards/_habits/{habitId}/logs`, `POST /_dashboards/_habits/{habitId}/archive`, `POST /_dashboards/_reminders`, `POST /_dashboards/_reminders/{reminderId}/complete|snooze|reschedule|skip|restart`; HTML fragments. | No external notification API, no automatic assignment API, and no new context-panel action tools. Existing `avatar_reminder_upsert` remains the only reminder mutation tool descriptor. | Repository/service/controller/tool registry tests plus startup; browser proof delegated. | `DECISION-20260529-05` | `dashboard-fragment-navigation.md` |
