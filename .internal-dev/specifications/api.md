...
| API-20260526-02 | Agent Skills management and assignment APIs (planned contract) | active | Expose APIs for skill metadata list/read/create/update/delete, file operations constrained to the root `skills/` repository, and agent-skill assignment/unassignment. MVP API contract scopes assignment to agent profiles only. | Planned `/api/skills/*` and `/api/agents/{agentId}/skills/*` surfaces with typed DTOs for metadata, draft/guided creation payloads, and assignment state. | Project-local `.agents/skills/`, user-home scopes, and project/job/task/workflow/chat/session layered assignment are deferred and must not appear as active API behavior. Script execution and trust/registry flows are out of MVP API scope. | Controller/API tests and docs updates when endpoints land; startup smoke for wiring changes. | `DECISION-20260526-03`, `DECISION-20260526-06` | `agent-skills-specification-reference.md` |
