...
| SP-20260529-05 | Habit, reminder inbox, and context widget composition | active | Habits/Trackers, Reminders/Alerts, and Dashboard Context use bounded row/list bodies, compact metric chips, small HTMX action forms, and explicit non-punitive/deferred-boundary wording. Dashboard Context renders descriptor chips as read-only data, not clickable action controls. | `AvatarDashboardComponents.habitsTrackers`, `remindersAlerts`, `dashboardContext` | Standard create/log/archive/complete/snooze/reschedule/skip interactions use HTMX. No broad JavaScript transport or external notification affordance is introduced. | Controller tests and delegated Playwright visual critique for desktop/mobile Phase 05 surfaces. | `DECISION-20260529-05` | `simplypages-avatar-layout-and-editing.md` |
...
| SP-20260531-01 | SlotKey/RenderContext reuse boundaries | active | Stable Home dashboard, dashboard widget chrome, static page, status strip, selector/detail, and repeated fragment shells should consider SimplyPages `Template`, `SlotKey`, and per-request `RenderContext`. Use slot-keyed reuse when only labels, counts, hrefs, chips, statuses, or bounded child fragments change. | `HomeDashboardTemplates`, package guides | Do not force SlotKeys into highly dynamic widget bodies, row/editor structures, or fragments where attribute-heavy request construction is clearer. New helper/key-bundle value types expose `.of(...)` factories and mutable component instances are not shared across requests. | Focused template/controller tests, `git diff --check`, startup, and delegated browser proof for touched UI. | `DECISION-20260531-01` | `simplypages-avatar-layout-and-editing.md`, `dashboard-fragment-navigation.md` |
