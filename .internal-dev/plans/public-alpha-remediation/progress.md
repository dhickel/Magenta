...
| `03-execution-history-streams` | `public-alpha-remediation/execution-history-streams` | in_progress | Codex | subplan-04 passed | Owns public execution contract, transcript preservation, SSE events, and job run submission. |
...
| bug-21 | `03-execution-history-streams` | validating | `subplan-05-schedule-reaction-template-validation.md` | implementer/parent: `mvn -Dtest=OrchestrationRuntimeTest,AgentOrchestrationControllerTest,OrchestrationControllerTest test`; static `AssignmentType.valueOf` scan confirms schedule/reaction runtime parsing is centralized in `AssignmentTemplateParser`; `git diff --check`; bounded startup reached `Started Magenta2Application` on port `35009` before timeout shutdown. First validator failed because the HTMX create-reaction form still selected `JOB_RUN`; fix pass changed the form default to `REPORT` and added regression coverage. | pending |
