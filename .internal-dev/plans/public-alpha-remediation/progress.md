...
| bug-03 | `04-workflow-authoring-runtime-js` | validating | `subplan-01-draft-editing-validation.md` | implementer/parent: `mvn -Dtest=WorkflowRunnerTest,OrchestrationControllerTest,PublicRunSubmissionControllerTest test`; static scan confirmed draft save paths use `saveDefinition` while validate/submit/run paths call `validateGraph`, `saveDefinitionValidated`, or `requireExecutable`; `git diff --check`; bounded startup reached `Started Magenta2Application` on port `40027` before timeout shutdown; pending validation subagent | pending |
