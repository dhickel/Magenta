...
| bug-14 | `03-execution-history-streams` | validating | `subplan-03-plan-sse-contract.md` | implementer: `mvn -Dtest=PublicRunSubmissionControllerTest,TaskStreamSupportTest test`; `git diff --check`; bounded startup. Confirmed `/api/plans/{planId}/runs/stream` preserves submit-to-agent assignment semantics and emits semantic SSE `submitted`/`failed` event names with no `TaskExecutionEvent` event name. Parent validation pending. | pending |
