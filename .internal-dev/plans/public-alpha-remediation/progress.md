...
| `04-workflow-authoring-runtime-js` | `public-alpha-remediation/workflow-authoring-runtime-js` | in_progress | Codex implementation subagent | bug-03 pending parent validation | Must preserve HTMX-first CRUD and narrow JS graph behavior. |
...
| bug-03 | `04-workflow-authoring-runtime-js` | validating | `subplan-01-draft-editing-validation.md` | implementer/parent: `mvn -Dtest=WorkflowRunnerTest,OrchestrationControllerTest,PublicRunSubmissionControllerTest test`; static scan confirmed draft save paths use `saveDefinition` while validate/submit/run paths call `validateGraph`, `saveDefinitionValidated`, or `requireExecutable`; `git diff --check`; bounded startup reached `Started Magenta2Application` on port `40027` before timeout shutdown. First validator failed because `/workflows` still loaded `workflows.js` and replaced the server HTMX editor; fix pass removed that page attachment while leaving the static JS file for later JS-island cleanup. | pending |
