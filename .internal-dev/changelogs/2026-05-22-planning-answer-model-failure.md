...
- Anonymous execution evidence tools now accept object-shaped `plan_report` and `plan_complete` entries, converting criterion/evidence objects into validator-readable strings and accepting `artifacts` as an alias for `artifactPaths`.
...
- Structured execution evidence normalization preserves the user-visible fields needed for validation but ignores extra metadata on evidence objects.
